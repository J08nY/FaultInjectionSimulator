...
int add_command(Command *c, size_t line_nr) {
    ...
    if (config.no_code_fault && (c->type & (BITFLIP | HAVOC | ZERO | SET)) && c->destination >= config.code_start && c->
        destination < config.code_end) {
